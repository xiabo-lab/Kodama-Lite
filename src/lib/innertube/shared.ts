...
/**
 * Ported from YTMLite's `src/lib/innertube/shared.ts` — the hand-rolled
 * raw-POST InnerTube client and its response-parsing helpers. The request
 * building and every `map*`/`collect*`/`read*` function below is verbatim;
 ...
 * subsystem doc comment in `src/lib/network.ts` for why that's the
 * pragmatic call here rather than a from-scratch Rust port.
 */
...
const VISITOR_DATA_STORAGE_KEY = "kl:ytm-visitor-data";
...
// and serves a degraded experience (notably: no synced lyrics).
